...
The required-reads hook trio forces Claude to have specific documents (style guides, CLAUDE.md, settings.json, per-project docs) in context before any Read, Write, Edit, or NotebookEdit tool call that touches a matching file. Read is in the matcher set so wildcard rules (CLAUDE.md, settings.json) fire on the first tool call of a session and so extension/filepath rules shape Claude's reading of source code, not just edits to it. The PreToolUse enforcer lives at `./pretooluse_required_reads.py`, the PostToolUse Read observer at `./posttooluse_read_observer.py`, and the PreCompact state reset at `./precompact_required_reads.py`.
...
A rule carries zero or one match criterion via two mutually exclusive fields. `extension` is a suffix test against the normalised path (e.g. `.py` fires on any file whose path ends with `.py`). `filepath` is a substring test against the normalised path (e.g. `/server/` fires on any file whose path contains `/server/`). A rule with neither field is a wildcard that matches every file. Both fields on a single rule is a load-time validation error and the rule is silently skipped. Globs were rejected as the match surface because shell-glob semantics surprise authors (no brace expansion, non-obvious bracket classes, recursive `**` differs from Bash) and every realistic rule collapses to one of these two intents. Match values are lowercased at load time so runtime comparison is a plain `endswith` / `in`.

Every rule is block. An unsatisfied matched rule denies the tool call until Claude Reads the target doc in the current session, batched into a single deny envelope listing every unsatisfied target so Claude can Read them all in one round and retry once. There is no inject mode; the previous two-mode design was dropped because the whole point of the feature is forcing the docs into the transcript rather than hiding them behind injected context that the model may or may not weight correctly.

Reads of any manifest-listed `read` target always passthrough regardless of which other rules would otherwise fire. This breaks self-target deadlocks (the `.md` rule pointing at `markdown.md`, which is itself an `.md` file would block the very Read that satisfies it) and ensures that loading required context is never gated on other required context, which would force unintuitive Read ordering. Write/Edit/NotebookEdit on a target doc still demand a prior Read; only Read is exempt.
