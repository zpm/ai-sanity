########################################################################################################################
# hooks/tests/test_required_reads_subprocess.py
#
# Subprocess integration tests for the required-reads hook trio (pretooluse, posttooluse, precompact)
########################################################################################################################
import os
import sys
import unittest
...
import tests.test_entry_subprocesses


class PreToolUseRequiredReadsSubprocessTestCase(
    tests.fixtures_required_reads.HomeOverrideEnvVarTestCaseMixin,
    unittest.TestCase
):

    """Subprocess integration tests for `pretooluse_required_reads.py`. Every rule is block-mode: the hook either
    denies with a required-reads message or passes through. Each test writes a manifest into the sandboxed home
    (via HOOK_TEST_HOME_OVERRIDE) and pipes a synthetic payload through the entry script."""
    ...
    def test_malformed_project_manifest_does_not_prevent_global_rule_from_firing(self):
        ...
        tests.fixtures_required_reads.RequiredReadsManifestFixtureBuilder.write_raw_manifest_body(
    ...
    def test_notebook_edit_with_matching_rule_emits_deny(self):
        ...
        tests.test_entry_subprocesses.HookEntryScriptInvocationHelper.assert_deny_decision(
            ...
            exit_code,
            parsed_stdout,
            expected_message_substring = "notebook-style.md"
        )

    def test_payload_missing_file_path_field_passes_through(self):

        malformed_edit_payload = tests.fixtures.PreToolUsePayloadFixtureBuilder.build_base_payload(
            tool_name = "Edit",
            tool_input = {}
        )
        exit_code, parsed_stdout = tests.test_entry_subprocesses.HookEntryScriptInvocationHelper.invoke_entry_script(
    ...
    def test_rule_with_missing_read_target_hard_fails_with_configuration_error(self):
        ...
        tests.test_entry_subprocesses.HookEntryScriptInvocationHelper.assert_deny_decision(
            ...
            exit_code,
            parsed_stdout,
            expected_message_substring = "configuration error"
        )
        permission_decision_reason_value = parsed_stdout["hookSpecificOutput"]["permissionDecisionReason"]
...
class PostToolUseReadObserverSubprocessTestCase(
    ...
    """Subprocess integration tests for `posttooluse_read_observer.py`. The observer is the feedback path that turns
    a Read tool call into satisfaction-flag writes so the PreToolUse hook stops demanding the same doc in the same
    session."""
    ...
    def test_read_of_required_doc_causes_next_pretooluse_edit_to_pass_through(self):
        ...
        tests.test_entry_subprocesses.HookEntryScriptInvocationHelper.invoke_entry_script(
            entry_script_filename = "posttooluse_read_observer.py",
    ...
    def test_non_read_tool_posttooluse_payload_passes_through(self):
        ...
        not_read_payload = tests.fixtures.PreToolUsePayloadFixtureBuilder.build_posttooluse_read_payload(
...
class PreCompactRequiredReadsSubprocessTestCase(
    ...
    """Subprocess integration tests for `precompact_required_reads.py`. The hook clears the current session's
    satisfaction flags so the PreToolUse hook re-demands required docs after compaction."""
    ...
    def test_precompact_clears_existing_satisfaction_flags_for_the_session(self):
        ...
        precompact_payload = tests.fixtures.PreToolUsePayloadFixtureBuilder.build_precompact_payload()
        tests.test_entry_subprocesses.HookEntryScriptInvocationHelper.invoke_entry_script(
            entry_script_filename = "precompact_required_reads.py",
            pretooluse_payload = precompact_payload
        )
        post_compact_edit_payload = tests.fixtures.PreToolUsePayloadFixtureBuilder.build_edit_payload(
            new_string_content = "x",
            file_path = os.path.join(self.sandboxed_home_abs_path, "main.py")
        )
        exit_code, parsed_stdout = tests.test_entry_subprocesses.HookEntryScriptInvocationHelper.invoke_entry_script(
